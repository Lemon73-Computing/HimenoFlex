...
internal static class Native256
{
    private const string NATIVE_LIBRARY = "himenoBMT_LARGE";
...
internal sealed class HimenoNative256 : HimenoBMT.IHimenoNative
{
    public void InitMT() => Native256.InitMT();
    public float Jacobi(int nn) => Native256.Jacobi(nn);
    public double Fflop(int mx, int my, int mz) => Native256.Fflop(mx, my, mz);
    public double Mflops(int nn, double cpu, double flop) => Native256.Mflops(nn, cpu, flop);
}
